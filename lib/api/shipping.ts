...
export class ShippingService {
  ...
  private validateAddress(address: any, type: string): void {
    ...
    const isValidServiceAreaPostalCode = isPostalCodeInServiceArea(postalCode);
    const addressTypeLabel = type === 'sender' ? 'pickup location (sender)' : 'delivery location (receiver)';
    const actionLabel = type === 'sender' ? 'Pickup' : 'Delivery';

    if (!parsedCity || !isAllowedServiceCity(city)) {
      throw new Error(
        `${actionLabel} is only supported in ${SERVICE_AREA_LABEL_SHORT}. Your ${addressTypeLabel} city "${city}" is not in our service area.`,
      );
    }

    if (!isValidServiceAreaPostalCode) {
      throw new Error(
        `The ${addressTypeLabel} postal code "${address.postal_code}" is not in our service area. ${actionLabel} is only available in ${serviceAreaPostalHint}.`,
      );
    }

    if (!postalMatchesCity(postalCode, city)) {
      throw new Error(
        `The ${addressTypeLabel} postal code "${address.postal_code}" does not belong to ${parsedCity}. Expected a ${cityHintForPostal(postalCode)} postal code.`,
      );
    }
