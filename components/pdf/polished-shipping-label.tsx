...
// Helper function to generate barcode pattern
const generateBarcodePattern = (trackingNumber: string) => {
  // Simple pattern generation based on tracking number
  const bars = [];
  const chars = trackingNumber.replace(/-/g, '');
  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    if (code % 3 === 0) {
      bars.push('thin', 'thin', 'medium');
    } else if (code % 3 === 1) {
      bars.push('medium', 'thin', 'thick');
    } else {
      bars.push('thin', 'thick', 'thin');
    }
  }
...
// Synchronous QR Code generation helper for PDF rendering
export const generateSyncQRCode = async (value: string): Promise<string> => {
  try {
    // Use the most compatible configuration for PDF rendering
    const config = {
    ...
    const qrDataUrl = await QRCode.toDataURL(value, config);
    
    // Validate the generated data URL
    if (qrDataUrl && qrDataUrl.startsWith('data:image/') && qrDataUrl.length > 100) {
      return qrDataUrl;
    } else {
      throw new Error('Invalid QR code data URL generated');
    }
  } catch (error) {
    console.warn('QR code generation failed, using fallback pattern:', error);
    
    // Create a simple QR code pattern as fallback
    const size = 90;
    const modules = 21; // Standard QR code module count
    const moduleSize = size / modules;
    
    // Generate a deterministic pattern based on the text
    let pattern = '';
    for (let i = 0; i < value.length; i++) {
      pattern += value.charCodeAt(i).toString(2).padStart(8, '0');
    }
    
    // Create SVG with pattern
    let svgContent = `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">`;
    svgContent += `<rect width="${size}" height="${size}" fill="#ffffff" stroke="#000000" stroke-width="1"/>`;
    
    // Add pattern squares
    for (let y = 0; y < modules; y++) {
      for (let x = 0; x < modules; x++) {
        const index = (y * modules + x) % pattern.length;
        if (pattern[index] === '1') {
          svgContent += `<rect x="${x * moduleSize}" y="${y * moduleSize}" width="${moduleSize}" height="${moduleSize}" fill="#000000"/>`;
        }
      }
    }
...
// QR Code component optimized for PDF rendering
const HighQualityQRCode: React.FC<{ value: string; preGeneratedDataUrl?: string }> = ({ 
  value, 
  preGeneratedDataUrl 
}) => {
  const [qrDataUrl, setQrDataUrl] = useState<string | null>(preGeneratedDataUrl || null);
  const [isGenerating, setIsGenerating] = useState(!preGeneratedDataUrl);

  // Create a simple QR code pattern as ultimate fallback
  const createSimpleQRPattern = (text: string) => {
    const size = 90;
    const modules = 21; // Standard QR code module count
    const moduleSize = size / modules;
  ...
  useEffect(() => {
    // If pre-generated URL is provided, use it immediately
    if (preGeneratedDataUrl) {
      setQrDataUrl(preGeneratedDataUrl);
      setIsGenerating(false);
      return;
    }

    // Otherwise, generate asynchronously for browser context
    if (!value) {
      setQrDataUrl(createSimpleQRPattern('NO_DATA'));
      setIsGenerating(false);
      return;
    }
    ...
    const generateAsync = async () => {
      try {
        ...
        const url = await generateSyncQRCode(value);
        setQrDataUrl(url);
      } catch (error) {
        console.error('Async QR generation failed:', error);
        setQrDataUrl(createSimpleQRPattern(value));
      } finally {
        setIsGenerating(false);
      }
    };
      ...
      <Image
        src={'data:image/svg+xml;base64,' + btoa(`
          <svg width="90" height="90" viewBox="0 0 90 90" xmlns="http://www.w3.org/2000/svg">
            <rect width="90" height="90" fill="#f8f9fa" stroke="#dee2e6" stroke-width="1"/>
            <rect x="15" y="15" width="60" height="60" fill="#e9ecef" stroke="#adb5bd" stroke-width="1"/>
            <text x="45" y="50" text-anchor="middle" font-family="Arial, sans-serif" font-size="8" fill="#6c757d">QR</text>
            <text x="45" y="65" text-anchor="middle" font-family="Arial, sans-serif" font-size="6" fill="#adb5bd">Loading...</text>
          </svg>
        `)}
        style={styles.qrCode}
        alt="QR Code Loading"
...
const logoDataUri = `data:image/svg+xml;base64,${btoa(`<svg width="163" height="33" viewBox="0 0 163 33" fill="none" xmlns="http://www.w3.org/2000/svg">
...
// Define consistent styles with improved layout and positioning
const styles = StyleSheet.create({
  ...
  // QR code below package details
  qrCodeContainer: {
    position: 'absolute',
    top: 120, // Position below package details box
    right: 0,
    alignItems: 'center',
  },
  ...
  // Service type box - positioned top right
  serviceContainer: {
    position: 'absolute',
    top: 0,
    right: 0,
  ...
  // Package details - positioned directly below STANDARD badge
  ...
  // Tracking section - bottom center
  trackingSection: {
    position: 'absolute',
    bottom: 30,
    left: 0,
...
export interface ShippingLabelData {
  ...
  shipDate: string;
  // Optional pre-generated QR code data URL for PDF rendering
  preGeneratedQRCode?: string;
}
...
// Component for rendering Barcode
const Barcode: React.FC<{ trackingNumber: string }> = ({ trackingNumber }) => {
  const pattern = generateBarcodePattern(trackingNumber);
  ...
  return (
    <View style={styles.barcode}>
      {pattern.map((barType, index) => {
        const barStyle = {
          ...styles.barcodeBar,
          ...(barType === 'thin' ? styles.barcodeBarThin : {}),
          ...(barType === 'medium' ? styles.barcodeBarMedium : {}),
          ...(barType === 'thick' ? styles.barcodeBarThick : {}),
        };
...
const QRCodeErrorBoundary: React.FC<{ children: React.ReactNode; fallback: React.ReactNode }> = ({ children, fallback }) => {
      ...
      <Page size={[288, 432]} style={styles.page}>
        ...
        <View style={styles.mainContent}>
          ...
          <View style={styles.serviceContainer}>
            <View style={styles.serviceBox}>
              <Text style={styles.serviceType}>STANDARD</Text>
            </View>
          </View>
        ...
        {/* FROM Section */}
          <View style={styles.fromSection}>
            ...
            {data.sender.phone && (
                <Text style={styles.contactLine}>Phone: {data.sender.phone}</Text>
            )}
            {data.sender.email && (
                <Text style={styles.contactLine}>Email: {data.sender.email}</Text>
            )}
          </View>
        </View>
        ...
        {/* TO Section */}
          <View style={styles.toSection}>
            ...
            {data.recipient.company && (
              <Text style={styles.contactName}>{data.recipient.company}</Text>
            )}
            <Text style={styles.addressLine}>{data.recipient.address}</Text>
            <Text style={styles.addressLine}>
              {data.recipient.city}, {data.recipient.state} {data.recipient.postalCode}
            </Text>
            
            <View style={styles.contactDetails}>
              <Text style={styles.contactLine}>Phone: {data.recipient.phone}</Text>
              <Text style={styles.contactLine}>Email: {data.recipient.email}</Text>
          </View>
        </View>
          ...
          {/* QR Code - positioned below package details box */}
          <View style={styles.qrCodeContainer}>
                ...
                <Image
                  src={'data:image/svg+xml;base64,' + btoa(`
                    <svg width="90" height="90" viewBox="0 0 90 90" xmlns="http://www.w3.org/2000/svg">
                      <rect width="90" height="90" fill="#f8f9fa" stroke="#dee2e6" stroke-width="1"/>
                      <rect x="15" y="15" width="60" height="60" fill="#e9ecef" stroke="#adb5bd" stroke-width="1"/>
                      <text x="45" y="45" text-anchor="middle" font-family="Arial, sans-serif" font-size="10" fill="#6c757d">QR</text>
                      <text x="45" y="60" text-anchor="middle" font-family="Arial, sans-serif" font-size="6" fill="#adb5bd">Error</text>
                    </svg>
                  `)}
                  style={styles.qrCode}
                  alt="QR Code Error"
            ...
            >
              <HighQualityQRCode 
                ...
                preGeneratedDataUrl={data.preGeneratedQRCode} 
              />
            </QRCodeErrorBoundary>
          </View>
        ...
        {/* Tracking section at bottom */}
        <View style={styles.trackingSection}>
          <Text style={styles.trackingHeader}>TRACKING CODE</Text>
          
          <View style={styles.barcodeContainer}>
            <Barcode trackingNumber={data.trackingNumber} />
          ...
          <Text style={styles.footerText}>
            Generated by Parcego Logistics Platform
          </Text>
          </View>
        </View>
      </Page>
